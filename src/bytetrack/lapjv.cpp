    ...
    int *free_rows, int *x, int *y, double *v)
{
    ...
    for (int *pfree_i = free_rows; pfree_i < free_rows + n_free_rows; pfree_i++) {
        ...
        if (j < 0)
            throw std::runtime_error("Error in _ca_dense(): j < 0");
        if (j >= static_cast<int>(n))
            throw std::runtime_error("Error in _ca_dense(): j >= n");
        while (i != *pfree_i) {
            i = pred[j];
            y[j] = i;
            ...
            k++;
            if (k >= n)
                throw std::runtime_error("Error in _ca_dense(): k >= n");
        }
    }
    LAPJV_CPP_FREE(pred);
    return 0;
